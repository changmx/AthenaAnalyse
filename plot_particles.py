...
def plot_distribution(axes,row,col,x,y,xlable,ylable):
	...
	axes[row][col].set_xlabel(xlable,fontsize=fig_fontsize)
	axes[row][col].set_ylabel(ylable,fontsize=fig_fontsize)
	axes[row][col].tick_params(labelsize=fig_fontsize)
	axes[row][col].tick_params(labelsize=fig_fontsize)
	# axes[row][col].grid()
...
if __name__ == '__main__':
	...
	filePath = r'E:\changmx\bb2019\distribution\2020_1112\1830_positron_turn300_bunch0_50000.csv'

	# fig,axes=plt.subplots(3,3,figsize=(25,15),dpi=300)
	fig,axes=plt.subplots(2,2)
	# fig,axes=plt.subplots(2,2,figsize=(25,15),dpi=300)
	...
	plot_distribution(axes,1,1,z,y,'z','y')
	...
	# plt.savefig(r'./'+'hourglassDistribution'+'.png')
	plt.show()

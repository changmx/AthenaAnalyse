...
def plot_tuneSpread(path,ax,resonanceOrder,xlim=[0,1],ylim=[0,1],resonanceKind='all'):

	phaseX,phaseY,nuX,nuY,tag = np.loadtxt(path, delimiter=',', skiprows=1, usecols=(0,1,2,3,4), unpack=True)
	l_nuX = list(nuX)
	l_nuY = list(nuY)
...
if __name__ == '__main__':
	...
	plot_tuneSpread(path,ax,8,resonanceKind='all')
